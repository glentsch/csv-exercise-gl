...
def write_error_file(f, errors):
    with open(f, "w+") as out_file:
        writer = csv.DictWriter(out_file, fieldnames=(OUTPUT_COL_LINE_NO, OUTPUT_COL_ERROR_MSG))
        writer.writeheader()
        for error in errors:
            writer.writerow({OUTPUT_COL_LINE_NO: error.line, OUTPUT_COL_ERROR_MSG: json.dumps(error.errors)})
