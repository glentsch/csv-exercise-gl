...
def main():
    ...
    csv_reader_service = CSVReaderService(
        queue,
        settings["output_directory"],
        settings["error_directory"],
        validator,
        transformation_service,
    )
    input_watcher_service = InputWatcherService(
        queue, settings["input_directory"], settings["processed_files"]
    )
