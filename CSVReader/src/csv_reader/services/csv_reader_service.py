from csv_reader.utils import read_document, write_error_file, write_success_file, delete_file
...
class CSVReaderService:
    def __init__(self, queue, output_directory, error_directory, validator, transformation_service):
        self.queue = queue
    ...
    def _handle_error(self, errors, f):
        if errors:
            error_file_name = join(self.error_directory, splitext(basename(f))[0] + ".csv")
            write_error_file(error_file_name, errors) 
    def _handle_success(self, successes, f):
        if successes:
            success_file_name = join(self.output_directory, splitext(basename(f))[0] + ".json")
